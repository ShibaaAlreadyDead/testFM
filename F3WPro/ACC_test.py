...
def uart_listen():
    ...
    while WORK:
        ...
        if b'ICCID SIM1:' in data and found_ccid:
            find_ccid(data)

        if b'TIME: alarm period: 1 day 1 hour 0 min 0 sec' in data:
            rotate()

        if b'ACC: start event' in data:
            stop_servo()

        if b'ACC: set NO motion, level 1' in data:
            set_default()
            count_stop +=1
            
        if count_stop >= MAX_count:
            stop_test()
