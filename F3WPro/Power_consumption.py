...
# set owner in admin911 and set 5 min Search in lk911fm
        ...
        if b'TIME: alarm period: 0 day 0 hour 5 min 0 sec' in data and found_string and not block_executed:
            b2b(imei)  
            time.sleep(10)
            change_5min(ccid, chrome_driver_path)
            block_executed = True
            
        if b'TIME: alarm period: 0 day 0 hour 5 minute 0 sec' in data and not found_string:
            found_string = True
            
# after 10 T enters, stop
            
        if b'Going to sleep' in data: 
